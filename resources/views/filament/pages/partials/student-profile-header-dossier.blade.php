...
<div class="fi-student-profile-dossier overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
    ...
    <div class="relative overflow-hidden border-b border-gray-100 dark:border-white/10">
        ...
        <div class="relative flex flex-col gap-4 p-4 sm:p-5 lg:flex-row lg:items-start lg:gap-5">
            {{-- Photo --}}
            <div class="flex shrink-0 justify-center lg:justify-start">
                @if ($photo && $photo->isImage())
                    <button
                        type="button"
                        class="js-media-preview-trigger group relative cursor-zoom-in overflow-hidden rounded-2xl shadow-lg ring-2 ring-white dark:ring-gray-800"
                    ...
                    >
                        <img
                            ...
                            alt="{{ $record->name }}"
                            class="h-24 w-20 object-cover transition duration-300 group-hover:scale-105 sm:h-28 sm:w-[5.5rem]"
                        />
                    </button>
                @else
                    <div class="flex h-24 w-20 flex-col items-center justify-center rounded-2xl border border-dashed border-primary-200/80 bg-white/80 shadow-sm dark:border-primary-500/25 dark:bg-white/5 sm:h-28 sm:w-[5.5rem]">
            ...
            <div class="min-w-0 flex-1 text-center lg:text-left">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center justify-center gap-2 lg:justify-start">
                            <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-800 ring-1 ring-emerald-500/20 dark:text-emerald-300">
                                <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                                {{ $record->status->label() }}
                            </span>
                            <span class="rounded-md bg-gray-100 px-2 py-0.5 font-mono text-[11px] font-semibold text-gray-700 dark:bg-white/10 dark:text-gray-300">
                                {{ $enrollment->enrollment_number }}
                            </span>
                        </div>

                        <h2 class="mt-2 text-xl font-bold tracking-tight text-gray-950 sm:text-2xl dark:text-white">{{ $record->name }}</h2>
                        ...
                        @if ($enrollment->hasIdCard())
                            <div class="flex flex-wrap items-center justify-center gap-1 lg:justify-end">
                                <button type="button" wire:click="openIdCardPreview" class="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-primary-500">
                                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0"/></svg>
                                    ID card
                                </button>
                                <a href="{{ $enrollment->idCardDownloadUrl() }}" class="inline-flex items-center rounded-lg bg-gray-100 px-2.5 py-1.5 text-xs font-semibold text-gray-700 transition hover:bg-gray-200 dark:bg-white/10 dark:text-gray-200 dark:hover:bg-white/15">
                                    Download
                                </a>
                                @if (auth()->user()?->hasRole(\App\Enums\RoleName::SuperAdmin->value))
                                    <button type="button" wire:click="regenerateIdCard" wire:confirm="Regenerate ID card with the latest student photo and details?" class="inline-flex items-center rounded-lg px-2.5 py-1.5 text-xs font-medium text-amber-700 transition hover:bg-amber-50 dark:text-amber-300 dark:hover:bg-amber-500/10">
                                        Regenerate
                                    </button>
                                @endif
                            </div>
                ...
                <div class="mt-4 grid gap-2 sm:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-gray-100 bg-white/80 p-3 dark:border-white/10 dark:bg-white/[0.03]">
                        ...
                        <div class="mt-0.5 flex flex-wrap items-center justify-center gap-1.5 lg:justify-start">
                ...
                <div class="mt-3 flex flex-wrap items-center justify-center gap-2 lg:justify-start">
                    @if ($record->date_of_birth)
                        <span class="inline-flex items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1 text-xs text-gray-700 dark:bg-white/10 dark:text-gray-300">
                            <span class="font-medium text-gray-500">DOB</span> {{ $record->date_of_birth->format('d M Y') }}
                        </span>
                    @endif
                    @if ($record->father_name)
                        <span class="inline-flex max-w-[12rem] items-center gap-1 truncate rounded-lg bg-gray-100 px-2.5 py-1 text-xs text-gray-700 dark:bg-white/10 dark:text-gray-300">
    ...
    <div class="overflow-x-auto bg-gray-50/80 px-3 py-2.5 dark:bg-white/[0.02] sm:px-4 sm:overflow-visible">
        <div class="fi-student-profile-dossier-stats flex min-w-max gap-2 sm:grid sm:min-w-0">
            @foreach ($items as $counter)
                ...
                @endphp
                <div @class([
                    'flex min-w-[7rem] shrink-0 items-center gap-2.5 rounded-xl bg-white px-3 py-2.5 shadow-sm ring-1 ring-gray-200/70 dark:bg-gray-900 dark:ring-white/10 sm:min-w-0',
