                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/10">
                        @foreach ($scopeChargeSummaries as $row)
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-white/5">
                                ...
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ (int) $row['student_count'] }}</td>
                                <td class="px-4 py-3 font-medium text-emerald-700 dark:text-emerald-400">₹{{ number_format((float) $row['paid_total'], 2) }}</td>
                                <td class="px-4 py-3 font-medium text-amber-700 dark:text-amber-300">₹{{ number_format((float) $row['pending_total'], 2) }}</td>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/10">
                        @foreach ($recentChargeSummaries as $row)
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-white/5">
                                ...
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $row['added_by'] ?? '—' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
