...
@if ($record->hasFile() && $record->isPreviewable())
    <div class="space-y-3">
        ...
        @if ($isPdf)
            <iframe
                src="{{ $record->staffPreviewUrl() }}"
                class="h-[min(70vh,42rem)] w-full rounded-xl border border-gray-200 dark:border-white/10"
                title="{{ $record->title }}"
            ></iframe>
        @else
            <a href="{{ $record->staffPreviewUrl() }}" target="_blank" rel="noopener" class="block">
                <img
                    src="{{ $record->staffPreviewUrl() }}"
                    alt="{{ $record->title }}"
                    class="max-h-[min(70vh,42rem)] w-full rounded-xl border border-gray-200 object-contain dark:border-white/10"
                >
            </a>
        @endif
    </div>
