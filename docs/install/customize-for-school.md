...
> **IT / server installs:** follow **[`server-checklist.md`](server-checklist.md)** every time (commands, `.env`, Web Push VAPID, cron, queue). This guide covers branding and day-to-day customize after the server is up.
>
> **All install docs:** [`README.md`](README.md)
...
> Full production checklist (Web Push, cron, queue, `.env` sections): **[`server-checklist.md`](server-checklist.md)**.
...
# Edit .env: DB_*, APP_URL, ADMIN_MOBILE, ADMIN_PASSWORD — see server-checklist.md
...
# 6. Web Push (production) — generate once, paste into .env
php artisan crm:webpush-vapid

# 7. Queue worker (required for WhatsApp campaigns)
php artisan queue:work
```

On production, run the queue worker under **Supervisor** or systemd so it stays running (see **Part 5 — Production deployment** below and **server-checklist.md**).
...
| 7. Web Push VAPID | `php artisan crm:webpush-vapid` → paste into `.env` (once per school) |
...
1. The install folder (`docs/install/` — start with `README.md`, then `server-checklist.md`, then `customize-for-school.md`)  
