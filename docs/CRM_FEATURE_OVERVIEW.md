...
| `whatsapp` | WhatsApp (Meta) | Templates, inbox, campaigns, automations, usage analytics, hooks from fees/attendance/homework/calls; **Parent fee notices** (manual per-student amount bulk WhatsApp, Fees not required). Staff ticks: inbox / bulk campaigns / fee notices (or full desk) | Strong ops; settings cleanup ongoing |
