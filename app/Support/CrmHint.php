...
class CrmHint
{
    ...
    private const HINTS = [
        ...
        'setup.whatsapp' => 'Map punch IN/OUT, post-call, and campaign batch settings to Meta-approved templates. Sync templates under Connection & Setup first.',
        'setup.meta_whatsapp' => 'Connect directly to Meta WhatsApp Cloud API. When enabled, all sends (campaigns, punch, homework, post-call) route through Meta — Pal Digital is only a legacy fallback when Meta is off.',
        ...
        'setup.biometric' => 'EasyTimePro writes punches to punch_logs on the same MySQL server. Match device employee ID to student roll number, then configure parent messages under META WhatsApp → Automations.',
